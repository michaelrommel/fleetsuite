...
///
/// The PREROUTING jhash DNAT rules are scoped to `ip daddr <secondary_ip>`
/// (the customer-facing VIP) so they only match customer INBOUND traffic and
/// never hijack a concentrator-INITIATED outbound IKE transiting this node
/// (P2, AGENTS.md).  All three rules carry the SAME fixed `seed` so proto-50,
/// UDP-500 and UDP-4500 from one source IP hash to the SAME concentrator;
/// with no seed nftables picks a random per-rule seed and IKE_SA_INIT, IKE_AUTH
/// and ESP split across nodes at N>1 (Architecture Decision #1).
///
/// Regenerated by ipsecpulse at boot and by ipsecscale on pool changes.
const JHASH_SEED: &str = "0xa5a5a5a5";
...
fn render_nft_nat(state: &State) -> String {
	...
	if n == 0 {
		...
		for rule in [
			format!("ip daddr {snat_ip} ip protocol 50 dnat ip to jhash ip saddr mod {n} seed {JHASH_SEED} map {map}"),
			format!("ip daddr {snat_ip} ip protocol udp udp dport 500 dnat ip to jhash ip saddr mod {n} seed {JHASH_SEED} map {map}"),
			format!("ip daddr {snat_ip} ip protocol udp udp dport 4500 dnat ip to jhash ip saddr mod {n} seed {JHASH_SEED} map {map}"),
		] {
