...
///
/// Delegates to the shared `ipseccore` renderer so ipsecpulse (boot) and
/// ipsecscale (VPN pool changes) produce byte-identical rules (same jhash seed).
fn render_nft_nat(state: &State) -> String {
	ipseccore::render_ipsec_nat(&state.instance_id, &state.eth0_secondary_ip, &state.vpn_ips)
}
